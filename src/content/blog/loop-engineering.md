...
date: 2026-06-25
excerpt: '从 Addy Osmani 的六块积木出发，拆解 AI Agent 中「让循环代替人工提示」的设计范式——循环怎么搭、怎么转、怎么收敛。'
---

## 前言

2026 年 6 月，一条推文在开发者圈引爆了 830 万次浏览。

PSPDFKit 创始人 Peter Steinberger 说：「我已经不再为编码 Agent 写 prompt 了。我现在做的事情是设计循环（Loop），让循环去提示 Agent。」几乎同一时间，Anthropic Claude Code 负责人 Boris Cherny 在开发者大会上分享了类似的理念：「我不再手动给 Claude 写提示词了，我运行着能让 Claude 自动编排任务的循环。」

最终，Google Chrome 团队的 **Addy Osmani** 发表长文，把这些散落的想法系统化为一个完整的框架，并正式命名为 **Loop Engineering（循环工程）**。

这个概念的源头其实很朴素。澳大利亚开发者 Geoffrey Huntley 写了一个三行 Bash 脚本——让 LLM 不断读自己的输出、自我修正、循环迭代——他管它叫 "Ralph Loop"。就是这么简单的东西，撕开了一个口子：如果我们不再一步一步地告诉 AI 该做什么，而是设计一套系统让它自己持续运转，会发生什么？

这篇文章从 Osmani 的六块积木出发，拆解 Loop Engineering 的核心设计。读完你会明白：循环系统由哪些组件构成、循环内部有哪些设计模式、以及循环失控时该怎么办。
...
用 Peter Steinberger 的话说：「定义目标，定义验证条件，定义失败了怎么处理。然后，就可以放手了。」

这不是一个新技术，而是一种**范式转移**。你的身份从「操作员」变成了「架构师」——你不再亲手操作每一步，而是设计整套运转机制。就像工厂的厂长不需要亲自拧每一颗螺丝，他设计的是流水线。

## 2. 六块积木：一个完整循环系统的解剖

Addy Osmani 把一个完整的 Loop 系统拆解为六块积木。它们不是六种技术，而是六种**角色**——每块积木回答一个特定的问题，组合起来就是一个能自主运转的 Agent 系统。
...
### 2.1 Automations：让循环自己跑起来

**Automations 解决的核心问题是：谁来驱动循环？**

答案是——不能让任何人来驱动，循环必须自己驱动自己。

一个 Automation 包含三样东西：一个**心跳**（让循环持续运转的节拍器）、一组**退出条件**（什么时候该停下来）、以及一个**触发器**（什么时候开始）。
...
这也是 Osmani 框架中最容易被低估的一块积木。很多人花了大量精力设计循环逻辑，却忽略了把项目知识喂给 Agent。结果就是：循环跑得很快，但每轮都在犯同样的低级错误——因为它根本不知道项目的约定。

### 2.4 Plugins & Connectors：打通外部世界

**Plugins & Connectors 解决的核心问题是：循环怎么和真实世界交互？**

LLM 本身只能生成文本。要让 Agent 真正「做事」——搜索网页、读写文件、调用 API、查询数据库、触发 CI 构建——需要一层连接器把外部系统的能力暴露给循环。

这是六块积木中**最依赖 Harness 层**的一块。Harness 负责工具集成和安全控制（详见 [Agent Harness 工程（上）](/blog/agent-harness-part1) §3），Loop 消费这些能力来驱动迭代。
...
### 2.6 Memory/State：记忆与状态持久化

**Memory/State 解决的核心问题是：循环中断了，怎么从上次继续？**

一个循环可能运行几分钟，也可能运行几天。中间可能因为进程崩溃、API 超时、甚至用户关闭电脑而中断。如果所有状态只存在内存里，中断就意味着一切从头开始。

Memory 积木把关键状态**持久化到外部存储**：
...
六块积木搭好了循环的骨架。接下来是循环内部怎么「转」的问题——每一轮迭代中，Agent 怎么思考、怎么行动、怎么从结果中学习。
...
但更重要的是 Loop Engineering 带来的**身份转变**。程序员正在从「写代码的人」变成「设计循环的人」——你不再亲手操作每一步，而是设计一套让系统自主运转的机制。Osmani 的六块积木给了这套机制一个清晰的骨架：Automations 驱动循环，Worktrees 隔离并行，Skills 固化知识，Connectors 连接世界，Sub-agents 分离职责，Memory 延续状态。
